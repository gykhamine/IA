...
def load_cnn2d(path):
    ...
    # Toutes les clés de tenseurs sont gardées plates ; les listes
    # (kernels, biases) sont reconstruites depuis les clés indexées.
    model = {k: v for k, v in header.items() if k != '_tensors'}
    list_bases = set()
    ...
    for base in list_bases:
        model.pop(base, None)  # supprimer l'éventuelle liste de shapes

    for k in header.get('_tensors', []):
        if k[-1].isdigit() and '_' in k:
            base, idx = k.rsplit('_', 1)
            if idx.isdigit() and base in list_bases:
