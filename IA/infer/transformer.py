...
def load_transformer(path):
    ...
    # Reconstruct model dict from header + tensors
    model = dict(header)  # start with all config values

    # Les clés qui sont des listes de tenseurs sont stockées dans le header
    # comme listes de shapes [shape0, shape1, ...] — il faut les remplacer
    # par les vrais tenseurs reconstruits depuis tensors.
    list_bases = set()
    ...
    # Effacer les entrées header qui seront reconstruites comme listes de tenseurs
        ...
        if '_' in k:
            ...
            if idx.isdigit():
                if base not in model:
                    model[base] = []
                # Insérer à la bonne position (les clés arrivent dans l'ordre du dict)
                model[base].append(tensors[k])
            else:
                model[k] = tensors[k]
        else:
            model[k] = tensors[k]
    model.pop('_tensors', None)
