...
def load_model(path: str) -> Tuple[Dict[str, Any], Dict[str, np.ndarray]]:
    ...
    with open(path, 'rb') as f:
        magic = f.read(4)
        if magic != MAGIC:
            raise ValueError(f"Fichier .gy invalide : magic={magic!r} (attendu {MAGIC!r})")
        vmaj, vmin = struct.unpack('<HH', f.read(4))
        if vmaj != VERSION_MAJOR:
            raise ValueError(f"Version .gy incompatible : {vmaj}.{vmin} (attendu {VERSION_MAJOR}.x)")
...
def model_info(path: str) -> Dict[str, Any]:
    ...
    with open(path, 'rb') as f:
        magic = f.read(4)
        if magic != MAGIC:
            raise ValueError(f"Fichier .gy invalide")
        vmaj, vmin = struct.unpack('<HH', f.read(4))
        header_len = struct.unpack('<I', f.read(4))[0]
        header = json.loads(f.read(header_len).decode('utf-8'))
