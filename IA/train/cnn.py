...
from ..ia_format import save_model
...
def train_cnn2d(X=None, y=None, input_shape=(5, 5), kernel_shape=(3, 3),
    ...
    for epoch in range(epochs):
        ...
        if avg_loss < best_loss:
            ...
            if num_conv_layers == 1:
                best_model = {
                    'kernel': kernels[0].copy(), 'bias': biases[0].copy(),
    ...
    if num_conv_layers == 1:
        model = {
            'kernel': kernel, 'bias': bias, 'w_fc': w_fc, 'b_fc': b_fc,
    ...
    # Separate tensors from config for ia_format
    tensors = {}
    header = {}
    for k, v in model.items():
        if isinstance(v, np.ndarray):
            ...
            # List of ndarrays → flatten with indexed keys
            for i, arr in enumerate(v):
                tensors[f'{k}_{i}'] = np.asarray(arr)
            header[k] = [list(a.shape) for a in v]  # save shapes for reconstruction
        else:
            header[k] = v
...
def train_cnn_nd(dimensions=4, volume_shape=None, kernel_shape=None,
    ...
    for i in range(len(X)):
        ...
        for layer_i in range(num_conv_layers):
            if num_conv_layers == 1:
                conv = convolve_nd(x, kernels[0]) + biases[0]
            else:
                conv = convolve_nd(x, kernels[layer_i]) + biases[layer_i]
            x = relu(conv)
        flat = x.flatten().reshape(1, -1)
